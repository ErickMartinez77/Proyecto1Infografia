...
function disparar(event)
    if event.phase == "ended" then
        ...
        if balas > 0 then
            ...
            if patos[1] ~= nil then
                d1 = math.sqrt( math.pow( patos[1].x-mira.x,2) + math.pow( patos[1].y-mira.y, 2 ))
            else 
                d1 = 100
            end
            if patos[2] ~= nil then
                d2 = math.sqrt( math.pow( patos[2].x-mira.x,2) + math.pow( patos[2].y-mira.y, 2 ))
            else 
                d2 = 1000
            end
            if patos[3] ~= nil then
                d3 = math.sqrt( math.pow( patos[3].x-mira.x,2) + math.pow( patos[3].y-mira.y, 2 ))
            else 
                d3 = 100
            end
            if patos[4] ~= nil then
                d4 = math.sqrt( math.pow( patos[4].x-mira.x,2) + math.pow( patos[4].y-mira.y, 2 ))
            else 
                d4 = 100
            end
            if patos[5] ~= nil then
                d5 = math.sqrt( math.pow( patos[5].x-mira.x,2) + math.pow( patos[5].y-mira.y, 2 ))
            else 
                d5 = 100
            end
            ...
            print(grupo_delantero2.numChildren)
        else 
